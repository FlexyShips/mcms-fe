...
export function LinkSentModal({ open, onOk }: { open: boolean; onOk: () => void }) {
  return (
    <AuthModal open={open}>
...
export function PasswordChangedModal({
  ...
  return (
    <AuthModal open={open}>
      ...
      <p className="mt-2 text-sm leading-relaxed text-[#8A8A8A]">Password change successful!</p>
      <AuthModalButton onClick={onProceed}>Proceed to Login</AuthModalButton>
    </AuthModal>
  );
}
